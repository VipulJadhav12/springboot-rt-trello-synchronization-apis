...
@Service
public class MediaEnggRTTicketImpl implements RTTicket {
	...
	public HashMap<String, RTTicket> getRTTickets() {
		...
		System.out.println("==================================");
		System.out.println("Tickets in Media Engineering Queue" + "\n" + "==================================");
		...
		try {
			...
			response = Unirest.get(configurationProperties.RT_SEARCH_URI)
					...
					.queryString("query", configurationProperties.RT_MEDIA_QUERY)
					.queryString("orderby", "+Created")
					.queryString("format", "l")
					.queryString("fields", configurationProperties.RT_MEDIA_QUEUE_FIELDS)
					.asString();
		...
		/*		HashMap<String, RTTickets> rtTickets  = new HashMap<>();
		try {
			...
			HttpPost httpget = new HttpPost(configurationProperties.RT_BASE_URI + "?user=" + configurationProperties.RT_USER +"&pass=" + configurationProperties.RT_PASSWORD + "&query=Queue='Media%20Engineering'AND(Status='new'ORStatus='open'ORStatus='stalled')" +
					"&orderby=+Created&format=l&fields=" + configurationProperties.RT_MEDIA_QUEUE_FIELDS);

			CloseableHttpResponse response = httpclient.execute(httpget);
			HttpEntity resEntity = response.getEntity();

			if (resEntity != null) {
				System.out.println("Media Engineering Response content length: " + resEntity.getContentLength());
				...
				while ((details = reader.readLine()) != null) {
					if(!details.endsWith("--") && details.length() != 0) {
						if(details.startsWith("id")) {
							rtTicketDetails = new StringBuilder(details);
							rtTicketId = details.replaceFirst("id: ticket/", "");
							continue;
						}
						rtTicketDetails.append("\n" + details);
					}
			...
			for(String ticketId : rtTickets.keySet()) {
				System.out.println(rtTickets.get(ticketId).toString());
			}
